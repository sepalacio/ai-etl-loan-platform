...
export class PipelineScheduler {
  ...
  /**
   * Every 5 minutes: pick up FAILED documents that have not been attempted
   * in the last 5 minutes and have retries remaining, then re-run the pipeline.
   * The pipeline resumes from the last known good step via the document's status.
   */
  ...
  async retryFailedDocuments(): Promise<void> {
    ...
    const eligible = candidates.filter((d) => d.retryCount < 3);
